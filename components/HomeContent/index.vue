				...
				<div class="p-4">
					...
					<div class="flex justify-between items-center mt-3">
						<span class="text-blue-600 font-bold"
							>{{ formatToRupiah(menu.price) }}</span
						>
					...
					<!-- Button -->
					<button
						class="mt-4 w-full py-2 flex justify-center gap-1 text-sm bg-blue-100 text-blue-600 font-semibold rounded-lg border border-blue-300 hover:bg-blue-200 cursor-pointer transition disabled:cursor-not-allowed disabled:bg-blue-50 disabled:text-blue-400"
						@click="addCart(menu)"
						:disabled="
							carts.some((cart) => cart?.name === menu?.name)
						">
						{{
							carts.some((cart) => cart?.name === menu?.name)
								? 'Sudah ditambahkan'
